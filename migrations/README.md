...
| 007 | `007_create_reporting_role.sql` | 2026-09-15 | Read-only `f1_reporting` for the dashboard: `select` on `marts` only. Run as `postgres` in the Supabase SQL editor (creating a role needs CREATEROLE, which `f1_pipeline` deliberately lacks), then `--mark-applied 007`. The load-bearing part is the **default privileges** — dbt recreates every mart on each build, and a recreated table inherits no grants, so without them the dashboard would work today and break on the next DAG run. Verified with a connection: 11/11 checks — reads `marts`, refused on `raw`, `staging`, insert, update, delete, create and drop |
