...
(1950 to 2026) pulled from a rate-limited public API, landed immutably in S3,
...
_Row counts as of 2026-09-19. They move on their own, since the scheduled run
ingests whatever has been raced since._

---

## What it answers

The star schema was designed **backwards from these questions**. The shape of
the source came last. Each question resolves against the model as built:

**Championship progress.** Standings after any round; how the points gap between
title contenders evolved race by race.
→ `fct_driver_standings`, `fct_constructor_standings` (periodic snapshots).

**Race results and reliability.** Finishing order, points, win and podium rates,
DNF rate by driver or constructor, and *why* cars retired.
...
One worked example. Reliability across seventy-seven seasons is the kind of
answer only the full backfill makes possible:
...
differ by up to 3.4 points, widest in the 1960s at 48.3% against 44.9%, which is
...
Ten visuals over two tabs. One dashboard serving both purposes serves neither: a
reviewer wants the seventy-seven-season sweep, someone following the
...
Look at the first card. **`pipeline_last_ran` and `data_last_changed` are five
days apart**, and that gap is expected. The warehouse upsert only writes when a
payload actually differs, so a run that succeeds and finds nothing new leaves
the data untouched. A freshness indicator built on "when did a row last change"
would call a healthy pipeline stale. `rpt_pipeline_freshness` exists to keep
...
map carries the scale better than any count could:
...
team, different driver" is a self-join on two keys instead of a bridge table:
...
Ordering by `driver_key` would have been arbitrary, since it is an MD5 hash, and
the first version of this chart did exactly that, leaving a reader unable to
tell which stacked segment belonged to whom.
...
across all races, because points are the outcome. So a pairing can lead the
head-to-head and still lose on points, which is the interesting case and the
reason the two are counted differently.
...
| Transformation | dbt Core | Tests, lineage and documentation live in the same files as the models |
| CI | GitHub Actions | `ruff` and `pytest` on every push |
| Orchestration | Airflow 3.0.2 (Docker) | Daily DAG, per-task retries, failure reporting. LocalExecutor, because a queue buys nothing on one machine |
...
every page, so an interrupted run resumes instead of restarting. Pass the
...
their own entity. Conformed dimensions do the work here. "Verstappen" means the
...
happens at exactly one circuit. The relationship belongs on the event dimension,
and repeating it on every fact that references the event would be redundant.
...
coalesce to when a key does not resolve, so an unmatched row is visible instead
of being dropped by an inner join.
...
**Testing.** Every dbt model declares and tests its grain. Tests that could not
meaningfully fail are treated as noise and left out; legitimately nullable
columns are documented instead of asserted.
...
race and sprint facts and compares it against the official standings. It is what
revealed that sprint points were missing entirely (2024 official 437 for
...
- **Code** (`ingestion/`, `dbt/`, `discovery/`, `migrations/`, `tests/`,
  ...
  data. Released under the **[MIT License](LICENSE)**, and the `LICENSE` file
